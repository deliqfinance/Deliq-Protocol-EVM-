...
interface IDefiRound {
    ...
    struct AccountDataDetails {
        ...
        uint256 actualTokeReceived; //Amount of DLQ that will be received
    }
    ...
    /// @notice publishes rates for the tokens. Rates are always relative to 1 DLQ. Can only be called once within Stage 1
    ...
    /// @dev uses the publishedRates, selected tokens, and amounts to determine what amount of DLQ is claimed
    ...
    /// @notice Given a balance, calculates how the the amount will be allocated between DLQ and Farming
